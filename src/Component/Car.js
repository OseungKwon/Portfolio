  ...
  return (
    <div>
      ...
      <MainStyle>
        ...
        <Content {...useScrollFadeIn("up", 1, 0.2)}>
...
또한, 팀원들의 코드를 리뷰, `}
        ...
        <Content2 {...useScrollFadeIn("up", 1, 0.2)}>
          <Text2>DEPL(QLICK)</Text2>
          <Point>설명</Point>
...
qna 댓글 기능을 개발하는 과정 속에서, 댓글이 정상적으로 업데이트가 되지 않는 문제가 발생하였는데, 이를 해결하기 위해 브라우저 동작 과정과 toast-ui editor setMarkdown 기능을 학습하고 이를 응용해 해결하였습니다.`}
        ...
        <Img3 {...useScrollFadeIn("up", 1, 0)}>
          <AliceCarousel
            ...
            infinite
          >
            <Img src={p3} alt="img" className="sliderimg" />
          </AliceCarousel>
        </Img3>
        <Content3 {...useScrollFadeIn("up", 1, 0.2)}>
          <Text2>Portfolio Page</Text2>
          <Point>설명</Point>
          {`초보 개발자, 혹은 사수가 없는 신입 개발자를 타겟으로, 코드 리뷰 서비스를 제공해주는 웹페이지.
qna 페이지, 피드 페이지, 블로그 페이지, 코드 리뷰 서비스 페이지로 이루어져 있습니다.`}
          <Point>진행 사항</Point>
